...
def excess_deaths_by_state():
    ...
    df_sum_pre_covid_week = df_group_by_date.where(df_group_by_date['Year']!=2020).groupby(
        ['State Abbreviation', 'Year', 'Week']).sum()
    df_avg_pre_covid_week = df_sum_pre_covid_week.groupby(
        ['State Abbreviation', 'Week']).mean()
    df_std_pre_covid_week = df_sum_pre_covid_week.groupby(
        ['State Abbreviation', 'Week']).std()
    ...
    df_covid_week['diff'] = (
        df_covid_week['Deaths 2020'] - df_covid_week['Deaths (5yr Avg)']
    ).values
    ...
    for state in df_covid_week['State'].unique():
        ...
        state_deaths[state] = {
            ...
            'weeks_deaths': df_covid_week.where(df_covid_week['State']==state)['Deaths 2020'].dropna().values,
